...
def test_export_mediapipe():
    ...
    pose = {"id": "test", "name": "Test", "joints": {"nose": [0.5, 0.1, 0.0], "l_shoulder": [0.4, 0.3, 0.0]}}
...
def test_new_poses_load():
    ...
    for pid in ["desk_standup_stretch", "couple_walk_holding_hands", "yoga_warrior_ii_side",
                 "studio_hand_on_hip_power", "beach_walk_side_profile"]:
        assert pid in files, f"Missing pose: {pid}"

def test_new_scenes_load():
    from poseguide.data.loader import load_scene, list_scene_files
...
if __name__ == "__main__":
    test_datasets_list(); print("OK datasets")
    test_scene_tagger_preset(); print("OK tagger preset")
    test_scene_tagger_desc(); print("OK tagger desc")
    test_export_mediapipe(); print("OK export mediapipe")
    test_export_coco(); print("OK export coco")
    test_embed_ranker_builds(); print("OK embed ranker")
    test_metrics_mrr(); print("OK metrics MRR")
    test_new_poses_load(); print("OK new poses")
    test_new_scenes_load(); print("OK new scenes")
