    ...
    def recommend(self, scene: dict, top_k: int = 3, subject_vector=None):
        ...
        for pose in self.poses:
            ...
            if subject_vector is not None and pose.get("joint_vector") is not None:
                ...
                cos = float(np.dot(pv[:n], sv[:n])) / (
                    float(np.linalg.norm(pv[:n])) * float(np.linalg.norm(sv[:n])) + 1e-9
                )
            ...
            ranked.append(
                {
                    ...
                    "tag_overlap": sorted(
                        {str(t).lower() for t in (pose.get("tags") or [])}
                        & {str(t).lower() for t in (scene.get("tags") or [])}
                    ),
